 ...
 */
package com.irurueta.android.navigation.inertial.test.old.estimators.pose
...
import com.irurueta.android.navigation.inertial.old.estimators.pose.RelativePoseEstimator
...
class RelativePoseEstimatorTest {
    ...
    fun startAndStop_estimatesCurrentPose() {
        ...
        val estimator = RelativePoseEstimator(
            ...
            poseAvailableListener = { _, timestamp, pose ->
                if (previousTimestamp < 0) {
                    previousTimestamp = timestamp
                    return@RelativePoseEstimator
                }
    ...
    private fun getCurrentLocation(): Location {
        val scenario = ActivityScenario.launch(LocationActivity::class.java).use {
            it.onActivity { activity ->
                this@RelativePoseEstimatorTest.activity = activity
    ...
    private fun logTranslation(pose: EuclideanTransformation3D) {
        pose.getTranslationPoint(translation)
        val distance = translation.distanceTo(origin)
        Log.d("RelativePoseEstimatorTest", "Translation: $distance meters")
    }
}