...
class GyroscopeSensorCollector2Test {
    ...
    fun constructor_whenRequiredParameters_setsDefaultValues() {
        ...
        val collector = GyroscopeSensorCollector2(context)
    ...
    fun constructor_whenAllParameters_setsExpectedValues() {
        ...
        val accuracyChangedListener =
            mockk<SensorCollector2.OnAccuracyChangedListener<GyroscopeSensorMeasurement, GyroscopeSensorCollector2>>()
        val measurementListener =
            mockk<SensorCollector2.OnMeasurementListener<GyroscopeSensorMeasurement, GyroscopeSensorCollector2>>()
    ...
    fun sensor_whenSensorTypeGyroscope_returnsExpectedValue() {
        ...
        val collector =
            GyroscopeSensorCollector2(contextSpy, GyroscopeSensorType.GYROSCOPE)
    ...
    fun start_whenNoSensorManager_returnsFalse() {
        ...
        val collector = GyroscopeSensorCollector2(contextSpy)
        assertEquals(0L, collector.startTimestamp)
    ...
    fun onSensorChanged_whenNoEvent_makesNoAction() {
        ...
        val measurementListener =
            mockk<SensorCollector2.OnMeasurementListener<GyroscopeSensorMeasurement, GyroscopeSensorCollector2>>()
        val collector =
            GyroscopeSensorCollector2(context, measurementListener = measurementListener)
    ...
    fun onSensorChanged_whenNoSensorType_makesNoAction() {
        ...
        val sensorManagerSpy = spyk(sensorManager)
        val sensorMock = mockk<Sensor>()
        every { sensorMock.type }.returns(Sensor.TYPE_ACCELEROMETER)
        every { sensorManagerSpy.getDefaultSensor(Sensor.TYPE_GYROSCOPE_UNCALIBRATED) }
            .returns(sensorMock)
        every { sensorManagerSpy.registerListener(any(), any<Sensor>(), any()) }.returns(true)
        val contextSpy = spyk(context)
    ...
    fun onSensorChanged_whenStartOffsetEnabled_setsStartOffset() {
        ...
        val sensorManagerSpy = spyk(sensorManager)
        val sensorMock = mockk<Sensor>()
        every { sensorMock.type }.returns(Sensor.TYPE_GYROSCOPE_UNCALIBRATED)
        every { sensorManagerSpy.getDefaultSensor(Sensor.TYPE_GYROSCOPE_UNCALIBRATED) }
            .returns(sensorMock)
        every { sensorManagerSpy.registerListener(any(), any<Sensor>(), any()) }.returns(true)
        val contextSpy = spyk(context)
