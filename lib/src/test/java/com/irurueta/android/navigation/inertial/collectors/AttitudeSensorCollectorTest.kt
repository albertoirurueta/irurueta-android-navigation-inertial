...
class AttitudeSensorCollectorTest {
    ...
    private lateinit var accuracyChangedListener:
            SensorCollector.OnAccuracyChangedListener<AttitudeSensorMeasurement, AttitudeSensorCollector>

    @MockK(relaxUnitFun = true)
    private lateinit var measurementListener:
            SensorCollector.OnMeasurementListener<AttitudeSensorMeasurement, AttitudeSensorCollector>
    ...
    fun sensor_whenSensorTypeAbsoluteAttitude_returnsExpectedValue() {
        ...
        val collector = AttitudeSensorCollector(context, AttitudeSensorType.ABSOLUTE_ATTITUDE)
    ...
    fun sensor_whenSensorTypeRelativeAttitude_returnsExpectedValue() {
        ...
        val collector = AttitudeSensorCollector(context, AttitudeSensorType.RELATIVE_ATTITUDE)
    ...
    fun sensorAvailable_whenSensorNotAvailable_returnsFalse() {
        every { sensorManager.getDefaultSensor(Sensor.TYPE_GAME_ROTATION_VECTOR) }
            .returns(null)
        every { context.getSystemService(Context.SENSOR_SERVICE) }.returns(sensorManager)

        val collector =
            AttitudeSensorCollector(context, AttitudeSensorType.RELATIVE_ATTITUDE)
    ...
    fun start_whenNoSensor_returnsFalse() {
        mockkStatic(SystemClock::class) {
            ...
            every { sensorManager.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR) }
                .returns(null)
    ...
    fun onSensorChanged_whenNoEvent_makesNoAction() {
        val collector = AttitudeSensorCollector(context, measurementListener = measurementListener)
    ...
    fun onSensorChanged_whenValidSensor_notifiesMeasurement() {
        ...
        // set start time
        val startTimestamp = System.nanoTime()
        setPrivateProperty(SensorCollector::class, collector, "startTimestamp", startTimestamp)
    ...
    fun onAccuracyChanged_whenNoSensor_makesNoAction() {
        val collector =
            AttitudeSensorCollector(context, accuracyChangedListener = accuracyChangedListener)
