...
//        every { contextSpy.getSystemService(Context.SENSOR_SERVICE) }.returns(sensorManagerSpy)
        ...
        verify(measurementListener, only()).onMeasurement(eq(collector), capture(attitudeSensorMeasurementCaptor), eq(0))
...
//        every { contextSpy.getSystemService(Context.SENSOR_SERVICE) }.returns(sensorManagerSpy)
        ...
        verify(bufferFilledListener, only()).onBufferFilled(collector)
//        verify(exactly = 1) { bufferFilledListener.onBufferFilled(collector) }
        verify(measurementListener, only()).onMeasurement(eq(collector), capture(attitudeSensorMeasurementCaptor), eq(buffer.size - 1))
//        val slot = slot<AttitudeSensorMeasurement>()
