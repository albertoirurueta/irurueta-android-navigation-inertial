...
class GyroscopeMeasurementGeneratorTest {
    ...
    @MockK
    private lateinit var processor: GyroscopeMeasurementGeneratorProcessor
    ...
    fun processorInitializationStarted_whenNoListener_makesNoAction() {
        ...
        val processor: GyroscopeMeasurementGeneratorProcessor? =
    ...
    @Test
    fun gyroscopeCollectorMeasurementListener_processesGyroscopeMeasurement() {
        val generator = GyroscopeMeasurementGenerator(context)
    ...
    fun gyroscopeAccuracyChangedListener_whenAccuracyReliable_makesNoAction() {
        ...
        val collector = mockk<GyroscopeSensorCollector>()
        gyroscopeAccuracyChangedListener.onAccuracyChanged(collector,
            SensorAccuracy.HIGH)
    ...
    fun numberOfProcessedGyroscopeMeasurements_returnsExpectedValue() {
        ...
        every { processor.numberOfProcessedGyroscopeMeasurements }
            .returns(numberOfProcessedGyroscopeMeasurements)

        // check
        assertEquals(
            numberOfProcessedGyroscopeMeasurements,
            generator.numberOfProcessedGyroscopeMeasurements
        )

        verify(exactly = 1) {
            processor.numberOfProcessedGyroscopeMeasurements
        }
    }
    ...
    fun gyroscopeBaseNoiseLevel_returnsExpectedValue() {
        ...
        every { processor.gyroscopeBaseNoiseLevel }
            .returns(gyroscopeBaseNoiseLevel)

        // check
        assertEquals(
            gyroscopeBaseNoiseLevel,
            generator.gyroscopeBaseNoiseLevel
        )

        verify(exactly = 1) { processor.gyroscopeBaseNoiseLevel }
    }
    ...
    fun gyroscopeBaseNoiseLevelAsMeasurement_returnsExpectedValue() {
        ...
        every { processor.gyroscopeBaseNoiseLevelAsMeasurement }
            .returns(angularSpeed)

        // check
        assertSame(
            angularSpeed,
            generator.gyroscopeBaseNoiseLevelAsMeasurement
        )

        verify(exactly = 1) { processor.gyroscopeBaseNoiseLevelAsMeasurement }
    }

    @Test
    fun getGyroscopeBaseNoiseLevelAsMeasurement_callsProcessor() {
        val generator = GyroscopeMeasurementGenerator(context)
        ...
        every { processor.getGyroscopeBaseNoiseLevelAsMeasurement(angularSpeed) }
            .returns(true)

        assertTrue(generator.getGyroscopeBaseNoiseLevelAsMeasurement(angularSpeed))

        verify(exactly = 1) {
            processor.getGyroscopeBaseNoiseLevelAsMeasurement(angularSpeed)
        }
    }
}