...
import com.irurueta.navigation.inertial.calibration.gyroscope.EulerQuaternionStepIntegrator;
import com.irurueta.navigation.inertial.calibration.gyroscope.MidPointQuaternionStepIntegrator;
import com.irurueta.navigation.inertial.calibration.gyroscope.RungeKuttaQuaternionStepIntegrator;
...
public class YuanQuaternionStepIntegratorTest {
    ...
    public void integrate_whenOneStep_computesExpectedResult() throws RotationException {
        ...
        final YuanQuaternionStepIntegrator integrator = new YuanQuaternionStepIntegrator();
        final Quaternion result1 = new Quaternion();
        final Quaternion result2 = new Quaternion();
        YuanQuaternionStepIntegrator.integrationStep(initialAttitude, wx, wy, wz, wx, wy, wz,
    ...
    public void integrate_whenMultipleSteps_computesExpectedResult()
        ...
        for (int t = 0; t < TIMES; t++) {
            ...
            for (int i = 0; i < NUM_SAMPLES; i++) {
                YuanQuaternionStepIntegrator.integrationStep(result1, wx, wy, wz, wx, wy, wz,
                        TIME_INTERVAL, result1);
    ...
    public void compareErrors_whenNoNoise_haveExpectedRelativeAccuracy() throws RotationException {
        ...
        for (int t = 0; t < TIMES; t++){
            ...
            final TrawnyQuaternionStepIntegrator trawnyIntegrator =
                    new TrawnyQuaternionStepIntegrator();
            ...
            for (int i = 0; i < NUM_SAMPLES; i++) {
                ...
                midPointIntegrator.integrate(result5, wx, wy, wz, wx, wy, wz, TIME_INTERVAL,
                        result5);
                rungeKuttaIntegrator.integrate(result6, wx, wy, wz, wx, wy, wz, TIME_INTERVAL,
                        result6);
            }
            ...
            final double[] resultEulerAngles = result3.toEulerAngles();
            final double[] resultMidPointEulerAngles = result4.toEulerAngles();
            final double[] resultRungeKuttaEulerAngles = result5.toEulerAngles();

            final double[] expectedEulerAngles = expectedResult.toEulerAngles();
            final double[] diffAngles1 = ArrayUtils.subtractAndReturnNew(
                    expectedEulerAngles, resultSuhAngles);
            final double[] diffAngles2 = ArrayUtils.subtractAndReturnNew(
                    expectedEulerAngles, resultTrawnyEulerAngles);
            final double[] diffAngles3 = ArrayUtils.subtractAndReturnNew(
                    expectedEulerAngles, resultYuanAngles);
            final double[] diffAngles4 = ArrayUtils.subtractAndReturnNew(
                    expectedEulerAngles, resultEulerAngles);
            final double[] diffAngles5 = ArrayUtils.subtractAndReturnNew(
                    expectedEulerAngles, resultMidPointEulerAngles);
            final double[] diffAngles6 = ArrayUtils.subtractAndReturnNew(
            ...
            Logger.getLogger(YuanQuaternionStepIntegratorTest.class.getName())
