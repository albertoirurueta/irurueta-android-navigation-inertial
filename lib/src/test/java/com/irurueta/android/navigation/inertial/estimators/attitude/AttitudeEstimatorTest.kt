...
class AttitudeEstimatorTest {
    ...
    private lateinit var attitudeAvailableListener: AttitudeEstimator.OnAttitudeAvailableListener

    @MockK(relaxUnitFun = true)
    private lateinit var accuracyChangedListener: AttitudeEstimator.OnAccuracyChangedListener
    ...
    fun start_whenNotRunning_callsInternalCollector() {
        ...
        // setup spy
        val attitudeSensorCollector: AttitudeSensorCollector? =
    ...
    fun attitudeSensorCollector_whenAccuracyChangedAndListener_notifies() {
        val estimator =
            AttitudeEstimator(context, accuracyChangedListener = accuracyChangedListener)
    ...
    fun attitudeSensorCollector_whenMeasurementListenerCoordinateTransformationAndEulerAnglesEnabled_notifies() {
        ...
        val measurementAttitude = getAttitude()
        val randomizer = UniformRandomizer()
        val headingAccuracy = Math.toRadians(randomizer.nextDouble(0.0, MAX_ANGLE_DEGREES))
            .toFloat()
        val timestamp = System.nanoTime()
        val measurement = AttitudeSensorMeasurement(
            measurementAttitude,
