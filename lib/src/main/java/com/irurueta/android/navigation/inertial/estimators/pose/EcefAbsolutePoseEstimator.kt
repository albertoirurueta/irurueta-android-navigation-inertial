 ...
 */
class EcefAbsolutePoseEstimator(
    val context: Context,
     ...
     */
    private val fusedCollector = AccelerometerGravityGyroscopeAndMagnetometerSyncedSensorCollector(
        context,
        ...
        measurementListener = { _, measurement ->
            if (useDoubleFusedAttitudeProcessor) {
                if (doubleFusedProcessor.process(measurement)) {
                    val poseTransformation = if (estimatePoseTransformation) {
                        doubleFusedProcessor.poseTransformation
                    } else {
                        null
                    }
                    notifyPose(
                ...
                if (fusedProcessor.process(measurement)) {
                    val poseTransformation = if (estimatePoseTransformation) {
                        fusedProcessor.poseTransformation
                    } else {
                        null
                    }
                    notifyPose(
     ...
     */
    private val accelerometerFusedCollector =
     ...
     */
    private val attitudeCollector = AttitudeAccelerometerAndGyroscopeSyncedSensorCollector(
        context,
        ...
        measurementListener = { _, measurement ->
            if (attitudeProcessor.process(measurement)) {
                val poseTransformation = if (estimatePoseTransformation) {
                    attitudeProcessor.poseTransformation
                } else {
                    null
                }
                notifyPose(
    ...
    fun interface OnPoseAvailableListener {
         ...
         */
        fun onPoseAvailable(
            estimator: EcefAbsolutePoseEstimator,
