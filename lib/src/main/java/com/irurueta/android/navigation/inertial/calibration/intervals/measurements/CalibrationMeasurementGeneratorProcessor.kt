 ...
 */
abstract class CalibrationMeasurementGeneratorProcessor<I> {
    /**
     ...
     */
    fun processAccelerometerMeasurement(measurement: AccelerometerSensorMeasurement) {
        val status = status
        ...
        processAccelerometerData(measurement, diffSeconds, sample)
        processSampleInInternalGenerator()
        numberOfProcessedAccelerometerMeasurements++

        if (!initialized && (status == Status.INITIALIZATION_COMPLETED
     ...
     */
    protected abstract fun processAccelerometerData(
