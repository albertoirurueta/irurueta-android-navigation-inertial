...
import com.irurueta.android.navigation.inertial.old.collectors.interpolators.AttitudeSensorMeasurementInterpolator
...
/**
 * Syncs attitude, accelerometer, gravity and gyroscope sensor measurements in case they arrive
 * with certain delay.
 *
 ...
 * when first measurement is received. True indicates the offset is computed, false assumes that
 * offset is null.
 * @property accelerometerStartOffsetEnabled indicates whether accelerometer start offset will be
 * computed when first measurement is received. True indicates the offset is computed, false assumes
 * that offset is null.
 * @property gravityStartOffsetEnabled indicates whether gravity start offset will be computed when
 * first measurement is received. True indicates the offset is computed, false assumes that offset
 * is null.
 * @property gyroscopeStartOffsetEnabled indicates whether gravity start offset will be computed when
 * first measurement is received. True indicates the offset is computed, false assumes that offset
 * is null.
...
class AttitudeAccelerometerGravityAndGyroscopeSensorMeasurementSyncer(
    ...
    private val attitudeSensorCollector = BufferedAttitudeSensorCollector(
        ...
        accuracyChangedListener = { _, accuracy ->
            accuracyChangedListener?.onAccuracyChanged(
                this@AttitudeAccelerometerGravityAndGyroscopeSensorMeasurementSyncer,
    ...
    override val syncedMeasurement =
        AttitudeAccelerometerGravityAndGyroscopeSyncedSensorMeasurement(
    ...
    private fun processMeasurements() {
        ...
        for (attitudeMeasurement in attitudeMeasurements) {
            ...
            if (foundAccelerometerMeasurements.isEmpty()) {
                if (hasPreviousAccelerometerMeasurement && hasPreviousGravityMeasurement
                    && hasPreviousGyroscopeMeasurement
                ...
                for (accelerometerMeasurement in foundAccelerometerMeasurements) {
                    ...
                    if (foundGravityMeasurements.isEmpty()) {
                        if (hasPreviousGravityMeasurement
                            && hasPreviousGyroscopeMeasurement
