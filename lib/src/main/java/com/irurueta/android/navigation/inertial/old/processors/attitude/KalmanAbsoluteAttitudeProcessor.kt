...
import com.irurueta.android.navigation.inertial.old.processors.attitude.KalmanAbsoluteAttitudeProcessor.Companion.DEFAULT_ACCELEROMETER_NOISE_STANDARD_DEVIATION
import com.irurueta.android.navigation.inertial.old.processors.attitude.KalmanAbsoluteAttitudeProcessor.Companion.DEFAULT_GYROSCOPE_NOISE_PSD
import com.irurueta.android.navigation.inertial.old.processors.attitude.KalmanAbsoluteAttitudeProcessor.Companion.DEFAULT_MAGNETOMETER_NOISE_STANDARD_DEVIATION
...
class KalmanAbsoluteAttitudeProcessor(
     ...
     * This is reused for performance reasons.
     */
    private val rotationAxis = DoubleArray(COMPONENTS)
    ...
    fun process(
        ...
        val result = if (isFirst) {
            ...
            if (!initialized) {
                ...
                val accelerometerNorm = accelerationTriad.norm
                ...
                copyFilterStateToAcceleration(state, accelerationState)
                copyFilterStateToRotationAxis(state, rotationAxisState)
    ...
    private fun fixState(state: Matrix) {
        q.normalize()
        q.toAxisAndRotationAngle(rotationAxis)

        copyQuaternionToFilterState(q, state)
        copyRotationAxisToFilterState(rotationAxis, state)
    }
