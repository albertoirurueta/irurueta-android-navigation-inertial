 ...
 * processed. In such case, average magnetometer norm should match expected Earth magnetic field
 * norm at current location.
 *
 ...
 */
class MagnetometerNormProcessor(
...
) : AccumulatedMeasurementProcessor<MagnetometerNormProcessor,
        AccumulatedMagneticFluxDensityMeasurementNoiseEstimator, MagneticFluxDensityUnit,
        MagneticFluxDensity, MagnetometerSensorMeasurement>(
    maxSamples,
    maxDurationMillis,
    stopMode
) {
    ...
    /**
     * Internal noise estimator of magnetic flux density magnitude measurements.
     ...
     * Measurement is converted from micro-tesla to tesla before processing.
     *
     ...
     */
    override fun process(measurement: MagnetometerSensorMeasurement): Boolean {
        // convert norm from micro-tesla to tesla
        val norm = measurement.toNorm()
        val normT = MagneticFluxDensityConverter.microTeslaToTesla(norm)
        return handleMeasurement(normT, measurement.timestamp)
    }
}